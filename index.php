...
if($redis) {
    ...
    foreach ($keys as $key) {
      ...
      // Nodes containing an item named __phpredismanager__ are also a key, not just a directory.
      // This means that creating an actual key named __phpredismanager__ will make this bug.
      $d[$key[count($key) - 1]] = array('__phpredismanager__' => true);
    ...
    function print_namespace($item, $name, $fullkey, $islast) {
      ...
      if (isset($item['__phpredismanager__'])) {
        // Unset it so we won't loop over it when printing this namespace.
        unset($item['__phpredismanager__']);
...
<h1 class="logo"><a href="?overview&amp;s=<?php echo $server['id']?>&amp;d=<?php echo $server['db']?>">phpRedisManager</a></h1>
